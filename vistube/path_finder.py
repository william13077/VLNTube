"""
Path-finding utilities: time-limited A*, nearest-free-point BFS,
collision-free line check, and RDP path simplification.
"""
...
class TimeLimitedBiAStarFinder(AStarFinder):
    ...
    def keep_running(self):
        """
        Check whether we have exceeded time or iteration constraints.
        Returns True to keep running, False to abort.
        """
        if self.runs >= self.max_runs:
            print(
                '{} hit the barrier of {} iterations without '
    ...
    def find_path(self, start: GridNode, end: GridNode, grid: Grid):
        ...
        # Failed to find path; return the nearest reachable node
        nodes = []
...
def main():
    ...
    for x, y in simplified_path:
        navigable_map_visual_simplified[y][x] = 0.5
